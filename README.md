...
We now run the same example by including a color code based on the received stellar insolation flux. We note than in both examples we customized the size of the planets through the *size_catalog_planets* keyword the maximum and minimum values of the color map through the *color_min* and *color_max* keyworkds (see the example2_met.ini and example2_insol.ini files).
